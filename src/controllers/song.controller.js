...
const getLikeCount = async (req, res) => {
    try {
        ...
        const count = await songService.getLikeCount(songId);
...
const getSongById = async (req, res) => {
    try {
        ...
        if (!song) {
            return res.status(404).json({ error: "Song not found1" });
        }
...
const getSongsByUser = async (req, res) => {
    try {
        ...
        const songs = await songService.getSongsByUser(userId);
        res.json(songs);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
const downloadSong = async (req, res) => {
    try {
      ...
      res.download(filePath, `MySic_${song.title}.mp3`);
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  };
